...
| [recommendation-evaluation/](recommendation-evaluation/) | 実装済み | **2026年**（今年）推薦システムの当日監視と事後分析。**画面・指標・テストは揃っているが実データは未通過**（[README](recommendation-evaluation/README.md)「実装の積み残し」） |

`recommendation-evaluation/` はこのリポジトリの守備範囲の拡張である
（去年データの分析 → 今年データの監視・評価）。
[PURPOSE.md](../PURPOSE.md) への反映は**済んでいる**（同 E-4）。
