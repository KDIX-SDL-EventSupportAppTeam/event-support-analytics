...
**判定は「その日までの累積」で行う（実装時に確定）。**
ビンゴカードは日をまたいで持ち越されるため、日ごとに独立して判定すると
両日参加者のライン数が過小になる。よって両日参加者の行では、
土曜の行に全期間の成立ライン数が入る（金曜の行は金曜終了時点の値）。
参加者単位のライン数が欲しい場合は `pid` ごとの **最大値** を取ること。

### カード関連カラムの集計時の注意（重要）

`n_card` / `n_rec_total` は**カード1枚の属性**であり、
両日参加者では同じ値が2行に重複して現れる。
**そのまま合計すると分母が二重計上される。**

| カラム | 参加者単位への畳み込み方 |
|---|---|
| `n_card` / `n_rec_total` | **max**（1回だけ数える） |
| `n_card_hit` / `n_rec_hit` | **sum**（同一ブースへの再訪問が存在しないため日ごとのヒットは互いに素） |
| `bingo_lines` | **max**（累積判定済みのため最終日の値が全期間の値） |

実装では `metrics.per_user_card_stats()` がこの畳み込みを担う。
推薦マスの効果測定・完走率は必ずこの関数を経由すること。
