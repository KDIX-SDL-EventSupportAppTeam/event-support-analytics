...
> **この表は 2026-08 時点のスナップショットである（issue #19）。**
> その後もテストアカウントの増減が続いており、`data/raw/dump_20260909_201004.json` では
> `bingo_card` = **4,688**（1名分16枚の削減）、`checkins` = 2,141（不変）。
> **分析は毎回その時点の最新ダンプから再生成する**ため、上表の絶対値ではなく
> 「`(default)` はイベント後も書き込みを受けている」という事実のほうが本質。
> `派生する数値` の 294名 / 8名も 2026-08 値であり、2026-09-09 ダンプでは 293名 / 9名。
> 結論に効く規模ではない（[FINDINGS.md](../../../FINDINGS.md) §10 項目2・3）。
